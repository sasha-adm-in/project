...
class YUICssCompressorPHPPort
{
    ...
    /**
     * A static method 
     * @param  string $content css contene
     * @return string          minified content
     */
    public static function minify($content){
        if(!self::$instance)
            self::$instance = new YUICssCompressorPHPPort();
        return self::$instance->run($content);
    }
    /**
    ...
    public function __construct($raise_php_limits = TRUE)
    {
        ...
        $this->memory_limit = 256 * 1048576; // 128MB in bytes
        $this->max_execution_time = 120; // 2 min
    ...
    public function run($css = '', $linebreak_pos = FALSE)
    {
        ...
        for ($i = 0, $n = count($css_chunks); $i < $n; $i++) {
            $css_chunks[$i] = $this->minimize($css_chunks[$i], $linebreak_pos);
     ...
     * @return string
     */
    private function minimize($css, $linebreak_pos)
    {
    ...
    private function replace_string($matches)
    {
        ...
        // minimize alpha opacity in filter strings
