...
class ModelCatalogProduct extends Model {
    public function getCategoryByProduct($product_id){
        $query = $this->db->query("SELECT DISTINCT(cd.category_id) AS category_id, cd.name AS name FROM " . DB_PREFIX . "product_to_category p2c LEFT JOIN " . DB_PREFIX . "category c ON p2c.category_id = c.category_id LEFT JOIN " . DB_PREFIX . "category_description cd ON c.category_id = cd.category_id WHERE c.parent_id = 0 AND p2c.product_id IN (" . $product_id . ")
					AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "'");

        if (!$query->num_rows) {
            $query = $this->db->query("SELECT DISTINCT(cd.category_id) AS category_id, cd.name AS name FROM " . DB_PREFIX . "product_to_category p2c LEFT JOIN " . DB_PREFIX . "category c ON p2c.category_id = c.category_id LEFT JOIN " . DB_PREFIX . "category_description cd ON c.category_id = cd.category_id WHERE p2c.product_id IN (" . $product_id . ")
						AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
        }
...
//		$product_data['sql']=$sql;
    ...
    public function getShortProductAttributes($product_id) {
        $product_attribute_group_data = array();
