...
class ControllerCommonSeoPro extends Controller {
	...
	public function index() {
		...
		if (!isset($this->request->get['_route_'])) {
			...
			//Fix for Opencart to understanding MFP manufacturer Seo-url
			$this->load->model('catalog/manufacturer');
			$manufacturers = $this->model_catalog_manufacturer->getManufacturers();
			foreach ($manufacturers as $manufacturer) {
				$manufacturers_urls[$this->str2url($manufacturer['name'])] = 'mfp=manufacturers[' . $manufacturer['manufacturer_id'] . ']';
			}
			//Fix for Opencart to understanding MFP manufacturer Seo-url

			$rows = array();
			foreach ($parts as $keyword) {
				if (strpos($this->cache_data['keywords'][$keyword], 'category_id') !== false) {
					$cat_info = $this->model_catalog_category->getCategory((int)str_replace('category_id=', '', $this->cache_data['keywords'][$keyword]));
					$status = $cat_info['status'];
				}
				if (isset($this->cache_data['keywords'][$keyword]) && (strpos($this->cache_data['keywords'][$keyword], 'category_id') === false || (strpos($this->cache_data['keywords'][$keyword], 'category_id') !== false && !isset($manufacturers_urls[$keyword])))) {
					$rows[] = array('keyword' => $keyword, 'query' => $this->cache_data['keywords'][$keyword]);
				}
				//Fix for Opencart to understanding MFP manufacturer Seo-url
				elseif (isset($manufacturers_urls[$keyword])) {
					$rows[] = array('keyword' => $keyword, 'query' => $manufacturers_urls[$keyword]);
				}
				//Fix for Opencart to understanding MFP manufacturer Seo-url
			}
					...
					if ($url[0] == 'category_id') {
						...
						if ($url[0] == 'mfp') {
							if (isset($this->request->get['mfp'])) {
								$this->request->get[$url[0]] = $url[1] . $this->request->get['mfp'];
							} else {
	...
	public function rewrite($link) {
		...
		foreach ($data as $key => $value) {
			switch ($key) {
				...
				//Fix for Opencart to understanding MFP manufacturer Seo-url
				case 'mfp':
					if (strpos($value, 'manufacturers') !== false) {
						$tmp_mfp = array();
						preg_match("/manufacturers\[\d{1,}\]/sim", $value, $tmp_mfp);
						$queries[] = 'mfp=' . $tmp_mfp[0];
						$data[$key] = substr(str_replace($tmp_mfp[0], '', $data[$key]), 1);
		...
		$manufacturers = $this->model_catalog_manufacturer->getManufacturers();
		foreach ($manufacturers as $manufacturer) {
			$manufacturers_urls['mfp=manufacturers[' . $manufacturer['manufacturer_id'] . ']'] = $this->str2url($manufacturer['name']);
		}
		//Fix for Opencart to understanding MFP manufacturer Seo-url
		...
		foreach($queries as $query) {
			...
			//Fix for Opencart to understanding MFP manufacturer Seo-url
			elseif (isset($manufacturers_urls[$query])) {
				$rows[] = array('query' => $query, 'keyword' => $manufacturers_urls[$query]);
			}
			//Fix for Opencart to understanding MFP manufacturer Seo-url
		}
