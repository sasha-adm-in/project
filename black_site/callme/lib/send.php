...
$to = "sun4eg77777@gmail.com"; //получатель уведомлений

// не трогать
$HTTP_HOST = parse_url ("http://".$_SERVER["HTTP_HOST"]); 
$HTTP_HOST = str_replace (array ("http://","www."), "", $HTTP_HOST["host"]);
$from = "info@".$HTTP_HOST; // отправитель. Если настраиваете smtp, не забудьте указать в $from вашу почту
...
$sms["log"] = "";
$sms["pss"] = "";
$sms["frm"] = "res.ua"; // добавьте новую подпись в смс-шлюзе и дождитесь апрува
$sms["num"] = "80674317400"; // ваш номер в формате без + (79218886622)
$sms["prv"] = "sms-fly.com"; // на выбор: sms.ru, infosmska.ru, bytehand.com, sms-sending.ru, smsaero.ru
...
if ($interval < 1) { // интервал отправки (сек)
	jsAnswer ("error", "c_error", "", "Повідомлення вже було надіслано");
} else {
	//$get_data = gF('os');
	$get_data = $_GET["cs"];
...
//get city 
		...
		$headers.= "From: RES.ua ЗАКАЗ<".$from.">\r\n"; 
		...
		jsAnswer("success", "c_success", "", "<center>OK</center>");
	} else {
		jsAnswer("error", "c_error", "", "<center>error</center>");
	}
}
?>