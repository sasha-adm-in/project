		...
		<?php foreach( $items as $item_group_id => $item ) { ?>
			<?php foreach( $item['childs'] as $childKey => $child ) { ?>
				<tr>
					...
					<td class="text-center">
						<select class="form-control sel" name="<?php echo $_attribsName; ?>[<?php echo $child['attribute_group_id']; ?>][items][<?php echo $child['attribute_id']; ?>][sort_order_values]">
...
<script>
function Set() {
    var o=document.getElementById('form1').getElementsByClassName('sel');
    var i;
...
<span onclick="Set();">Сортировка значений</span>
