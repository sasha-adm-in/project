          ...
          <tbody id="module-row<?php echo $module_row; ?>">
            <tr>
              ...
              <td class="left"><input type="text" name="slideshow_module[<?php echo $module_row; ?>][width]" value="<?php echo $module['width']; ?>" size="3" />
                ...
                <?php } ?></td>
              <td class="left">
                <select name="slideshow_module[<?php echo $module_row; ?>][layout_id]" class="layout">
                ...
                <div class="category_info" style="display: <?php echo $module['layout_id'] == 3 ? 'block' : 'none'; ?>;">
                    <input style="width: 90%;" type="text" value="<?php echo ($module['layout_id'] == 1) ? 'main' : $module['category_name']; ?>" placeholder="Введите название категории" name="slideshow_module[<?php echo $module_row; ?>][category_name]" />
                    <input type="hidden" value="<?php echo ($module['layout_id'] == 1) ? 'main' : $module['category_id']; ?>" name="slideshow_module[<?php echo $module_row; ?>][category_id]" />     
                </div>
              </td>
...
$(document).on('change', '.layout', function() {
    if ($(this).val() == 3) {
        $(this).parent().find(".category_info").show();
        $(this).parent().find(".category_info input").val('');
    } else if ($(this).val() == 1) {
        $(this).parent().find(".category_info").hide();
        $(this).parent().find(".category_info input").val('main');
    } else {
        $(this).parent().find(".category_info input").val('');
        $(this).parent().find(".category_info").hide();
    } 
...
$(document).on("keydown.autocomplete", "input[name*='category_name']", function(e) {
    ...
    _this.autocomplete({
    	...
    	select: function(event, ui) {   
            _this.parent().find('input[name*=\'category_name\']').val(ui.item.label);
            _this.parent().find('input[name*=\'category_id\']').val(ui.item.value);
...
function addModule() {	
	...
	html += '    <td class="left"><select name="slideshow_module[' + module_row + '][layout_id]" class="layout">';
	<?php foreach ($layouts as $layout) { ?>
	html += '      <option value="<?php echo $layout['layout_id']; ?>"><?php echo addslashes($layout['name']); ?></option>';
	<?php } ?>
	html += '    </select> \
                    <div class="category_info" style="display: none;"> \
                        <input style="width: 90%;" type="text" placeholder="Введите название категории" name="slideshow_module[' + module_row + '][category_name]" /> \
                        <input type="hidden" name="slideshow_module[' + module_row + '][category_id]" /> \
                    </div> \
                </td>';
