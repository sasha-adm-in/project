...
class ControllerModuleMegaFilter extends Controller {
	...
	private function _init( $tab ) {
		...
		if( ! $curr_ver || version_compare( $curr_ver, $this->_version, '<' ) || $this->_isOldMFilterPlus() ) {			
			...
			$add_routes = array(
				'Filter' => 'module/mega_filter/results',
		...
		} else if( ! file_exists( DIR_SYSTEM . '../catalog/view/theme/res_final/template/module/mega_filter.tpl' ) ) {
		...
		if( is_readable( __FILE__ ) ) {
			...
			$source = filemtime( DIR_SYSTEM . '../catalog/view/theme/res_final/template/module/mega_filter.tpl' );
			...
			foreach( $files as $id => $file ) {
				...
				if( $theme == 'res_final' || ! is_readable( $file ) ) {
					unset( $files[$id] );
				} else {
					$time = filemtime( $file );
	...
	private function _updateCss( $settings ) {
		...
		file_put_contents( DIR_SYSTEM . '../catalog/view/theme/res_final/stylesheet/mf/style-2.css', $css );
	}
	
	private function _writableCss() {
		$path = DIR_SYSTEM . '../catalog/view/theme/res_final/stylesheet/mf/style-2.css';
	...
	public function install() {		
		...
		foreach( $this->model_localisation_language->getLanguages() as $language ) {
			$titles[$language['language_id']] = 'Filter';
		}
