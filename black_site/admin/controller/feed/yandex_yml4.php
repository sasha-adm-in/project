...
class ControllerFeedYandexYml4 extends Controller {
	...
	public function index() {
		...
		$this->load->language('feed/yandex_yml4');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate($this->request->post))) {
			...
			foreach($this->request->post as $key=>$val) {
				$key = str_replace('yandex_yml_', 'yandex_yml4_', $key);
				$to_save[$key] = $val;
			}
			$this->model_setting_setting->editSetting('yandex_yml4', $to_save);
		...
		$this->data['cron_path'] = 'php '.realpath(DIR_CATALOG.'../export/yandex_yml4.php');
		$this->data['entry_export_url'] = $this->language->get('entry_export_url');
		$this->data['export_url'] = HTTP_CATALOG.'export/yandex_yml4.xml';
		...
		$this->data['breadcrumbs'][] = array(
			'href'      => $this->url->link('feed/yandex_yml4', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title'),
			'separator' => ' :: '
		);

		$this->data['action'] = $this->url->link('feed/yandex_yml4', 'token=' . $this->session->data['token'], 'SSL');
		...
		*/
		...
		if (isset($this->request->post['yandex_yml_delivery_cost'])) {
			...
			$this->data['yandex_yml_delivery_cost'] = $this->config->get('yandex_yml4_delivery_cost');
		}

		if (isset($this->request->post['yandex_yml_pricefrom'])) {
			$this->data['pricefrom'] = $this->request->post['yandex_yml_pricefrom'];
		} else {
			$this->data['pricefrom'] = $this->config->get('yandex_yml4_pricefrom');
		}
		
		if (isset($this->request->post['yandex_yml_currency'])) {
			$this->data['yandex_yml_currency'] = $this->request->post['yandex_yml_currency'];
		} else {
			$this->data['yandex_yml_currency'] = $this->config->get('yandex_yml4_currency');
		}
		...
		if (isset($this->request->post['yandex_yml_blacklist'])) {
			...
			$blacklist = explode(',', $this->config->get('yandex_yml4_blacklist'));
		}
		...
		} elseif ($this->config->get('yandex_yml4_unavailable')) {
			$this->data['yandex_yml_unavailable'] = $this->config->get('yandex_yml4_unavailable');
		} else {
		...
		} elseif ($this->config->get('yandex_yml4_in_stock')) {
			$this->data['yandex_yml_in_stock'] = $this->config->get('yandex_yml4_in_stock');
		} else {
		...
		if (isset($this->request->post['yandex_yml_out_of_stock'])) {
			...
			$this->data['yandex_yml_out_of_stock'] = $this->config->get('yandex_yml4_out_of_stock');
		} else {
		...
		if (isset($this->request->post['yandex_yml_pickup'])) {
			...
			$this->data['yandex_yml_pickup'] = $this->config->get('yandex_yml4_pickup');
		}
		...
		if (isset($this->request->post['yandex_yml_sales_notes'])) {
			...
			$this->data['yandex_yml_sales_notes'] = $this->config->get('yandex_yml4_sales_notes');
		}

		if (isset($this->request->post['yandex_yml_store'])) {
			$this->data['yandex_yml_store'] = $this->request->post['yandex_yml_store'];
		} else {
			$this->data['yandex_yml_store'] = $this->config->get('yandex_yml4_store');
		}
		...
		if (isset($this->request->post['yandex_yml_numpictures'])) {
			...
			$this->data['yandex_yml_numpictures'] = $this->config->get('yandex_yml4_numpictures');
		}
		...
		} elseif ($this->config->get('yandex_yml4_attributes') != '') {
			$this->data['yandex_yml_attributes'] = explode(',', $this->config->get('yandex_yml4_attributes'));
		} else {
		...
		if (isset($this->request->post['yandex_yml_adult'])) {
			...
			$this->data['yandex_yml_adult'] = $this->config->get('yandex_yml4_adult');
		}
		if (isset($this->request->post['yandex_yml_manufacturer_warranty'])) {
			...
			$this->data['yandex_yml_manufacturer_warranty'] = $this->config->get('yandex_yml4_manufacturer_warranty');
		}
		if (isset($this->request->post['yandex_yml_country_of_origin'])) {
			...
			$this->data['yandex_yml_country_of_origin'] = $this->config->get('yandex_yml4_country_of_origin');
		}
		if (isset($this->request->post['yandex_yml_attr_vs_description'])) {
			...
			$this->data['yandex_yml_attr_vs_description'] = $this->config->get('yandex_yml4_attr_vs_description');
		}
		...
		if (isset($this->request->post['yandex_yml_product_rel'])) {
			...
			$this->data['yandex_yml_product_rel'] = $this->config->get('yandex_yml4_product_rel');
		}
		if (isset($this->request->post['yandex_yml_product_accessory'])) {
			...
			$this->data['yandex_yml_product_accessory'] = $this->config->get('yandex_yml4_product_accessory');
		}
		...
		} elseif ($this->config->get('yandex_yml4_color_options') != '') {
			$this->data['yandex_yml_color_options'] = explode(',', $this->config->get('yandex_yml4_color_options'));
		} else {
			$this->data['yandex_yml_color_options'] = array();
		}
		if (isset($this->request->post['yandex_yml_size_options'])) {
			$this->data['yandex_yml_size_options'] = $this->request->post['yandex_yml_size_options'];
		} elseif ($this->config->get('yandex_yml4_size_options') != '') {
			$this->data['yandex_yml_size_options'] = explode(',', $this->config->get('yandex_yml4_size_options'));
		} else {
			$this->data['yandex_yml_size_options'] = array();
		}
		if (isset($this->request->post['yandex_yml_size_units'])) {
			$this->data['yandex_yml_size_units'] = $this->request->post['yandex_yml_size_units'];
		} elseif ($this->config->get('yandex_yml4_size_units') != '') {
			$this->data['yandex_yml_size_units'] = unserialize($this->config->get('yandex_yml4_size_units'));
		} else {
		...
		if (isset($this->request->post['yandex_yml_optioned_name'])) {
			...
			$this->data['yandex_yml_optioned_name'] = $this->config->get('yandex_yml4_optioned_name');
		}
		...
		} elseif ($this->config->get('yandex_yml4_categories') != '') {
			$this->data['yandex_yml_categories'] = explode(',', $this->config->get('yandex_yml4_categories'));
		} else {
			$this->data['yandex_yml_categories'] = array();
		}
		if (isset($this->request->post['yandex_yml_categ_mapping'])) {
			$this->data['yandex_yml_categ_mapping'] = $this->request->post['yandex_yml_categ_mapping'];
		} elseif ($this->config->get('yandex_yml4_categ_mapping') != '') {
			$this->data['yandex_yml_categ_mapping'] = unserialize($this->config->get('yandex_yml4_categ_mapping'));
		} else {
			$this->data['yandex_yml_categ_mapping'] = array();
		}
		if (isset($this->request->post['yandex_yml_manufacturers'])) {
			$this->data['yandex_yml_manufacturers'] = $this->request->post['yandex_yml_manufacturers'];
		} elseif ($this->config->get('yandex_yml4_manufacturers') != '') {
			$this->data['yandex_yml_manufacturers'] = explode(',', $this->config->get('yandex_yml4_manufacturers'));
		} else {
	...
	private function validate($data) {
		if (!$this->user->hasPermission('modify', 'feed/yandex_yml4')) {
