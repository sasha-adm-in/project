...
//        echo "</pre>";
        ...
        if (isset($this->session->data['delivery_payment'])) {
            $delivery_sum = $this->session->data['delivery_payment'];
        } else {
            $delivery_sum = 0;
        }
...
//        }
        ...
        if ($this->customer->isLogged()) {
            $discounts = $this->model_checkout_order->getDiscounts();
            
            $orders = $this->model_checkout_order->getCustomerOrders($this->customer->getId());
            ...
            foreach ($discounts as $discount) {
                if ($sum >= $discount['discount_from'] && $sum <= $discount['discount_to']) {
                    $percent = $discount['percent']; break;
                }
            }           
            
            $total_sum = $total_sum - ($total_sum * ($percent / 100));
            
            $this->data['total_sum'] = number_format(round($total_sum + $delivery_sum, 2), 2, '.', '');
        }

        $this->session->data['total_sum'] = number_format(round($total_sum + $delivery_sum, 2), 2, '.', '');
		...
		$this->data['text_cart_order'] = $this->language->get('text_cart_order');
		$this->data['text_cart_code'] = $this->language->get('text_cart_code');
		$this->data['text_cart_codebtn'] = $this->language->get('text_cart_codebtn');
		$this->data['text_cart_confirmorder'] = $this->language->get('text_cart_confirmorder');
		$this->data['text_payment_totalsum'] = $this->language->get('text_payment_totalsum');
		$this->data['text_payment_deltov'] = $this->language->get('text_payment_deltov');
