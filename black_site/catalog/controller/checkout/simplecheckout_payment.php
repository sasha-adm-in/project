...
class ControllerCheckoutSimpleCheckoutPayment extends Controller 
{
    public function getCities() 
    {
        ...
        {
            $this->session->data['delivery_region'] = $this->request->post['region'];
    ...
    public function getWarenhouses() 
    {
        $this->load->model('checkout/nova_poshta');	
        ...
        {
            $this->session->data['delivery_city'] = $this->request->post['city'];
            
            $warenhouses = $this->model_checkout_nova_poshta->getWarenhouses($this->request->post['city']);
            
            exit(json_encode($warenhouses));
        }
    }
    ...
    public function calculate() 
    {
        ...
        if (isset($this->request->post['warenhouse']) && isset($this->request->post['redelivery'])) {
            $this->session->data['delivery_warenhouse'] = html_entity_decode($this->request->post['warenhouse'], ENT_QUOTES, 'UTF-8');
            $this->session->data['redelivery'] = (int)$this->request->post['redelivery'];
        }        
        
        $volume = 0; // сумарный объем
        $weight = 0; // сумарный вес
        $price  = 0; // сумарная цена
        ...
        $size = round(pow($volume, 1/3), 2);
        
        if ($weight == 0) {
            $weight = '';
        }
        
        $cost = $this->model_checkout_nova_poshta->getDeliveryCost($this->session->data['delivery_city'], $weight, $size, $size, $size, round($price, 2), 4, 1, $this->session->data['redelivery']);
        
        $this->session->data['delivery_payment'] = (string)$cost;
           
        //exit(json_encode(array('cost' => $cost)));
        
        //$this->log->write(print_r((string)$cost, true));
//        $this->log->write($size.' <-size | w-> '.$weight.' p-> '.round($price));
    }
    ...
    public function index() 
    {
        ...
        $this->data['regions'] = $this->model_checkout_nova_poshta->getRegions();
        
        if (isset($this->session->data['delivery_payment'])) {
            $this->calculate();
            
            $this->data['total_delivery'] = array(
                'region'     => $this->session->data['delivery_region'],
                'city'       => $this->session->data['delivery_city'],
                'warenhouse' => $this->session->data['delivery_warenhouse'],
                'cost'       => $this->session->data['delivery_payment']
            );
        }        
		...
		$this->data['text_payment_deliver'] = $this->language->get('text_payment_deliver');
		$this->data['text_deliver_oblc'] = $this->language->get('text_deliver_oblc');
		$this->data['text_deliver_obl'] = $this->language->get('text_deliver_obl');
		$this->data['text_deliver_gor'] = $this->language->get('text_deliver_gor');
		$this->data['text_deliver_otd'] = $this->language->get('text_deliver_otd');
		$this->data['text_deliver_sum'] = $this->language->get('text_deliver_sum');
		$this->data['text_deliver_gorc'] = $this->language->get('text_deliver_gorc');
		$this->data['text_deliver_otdc'] = $this->language->get('text_deliver_otdc');
