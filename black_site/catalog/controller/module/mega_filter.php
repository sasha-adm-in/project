...
class ControllerModuleMegaFilter extends Controller {
	...
	protected function index( $setting ) {
		...
		if( $mijo_shop ) {
			...
			if( file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/stylesheet/mf/style.css') ) {
				...
				MijoShop::get()->addHeader(JPATH_MIJOSHOP_OC.'/catalog/view/theme/res_final/stylesheet/mf/style.css');
			}
			
			MijoShop::get()->addHeader(JPATH_MIJOSHOP_OC.'/catalog/view/theme/res_final/stylesheet/mf/style-2.css');
		} else {
			...
			if( file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/stylesheet/mf/style.css') ) {
				...
				$this->document->addStyle('catalog/view/theme/res_final/stylesheet/mf/style.css?v'.$this->data['_v']);
			}
			
			$this->document->addStyle('catalog/view/theme/res_final/stylesheet/mf/style-2.css?v'.$this->data['_v']);
		}

		if( ! empty( $this->request->get['path'] ) ) {
			$this->data['current_category'] = $this->url->link('product/category', 'path=' . $this->request->get['path']);
		}
		...
		if( file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/mega_filter.tpl') ) {
			...
			$this->template = 'res_final/template/module/mega_filter.tpl';
		}
	...
	public function results() {
		...
		if( file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/special.tpl') ) {
			...
			$this->template = 'res_final/template/product/special.tpl';
		}
