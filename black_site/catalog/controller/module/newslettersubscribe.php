		   ...
		   if($this->config->get('newslettersubscribe_registered') and $this->model_account_newslettersubscribe->checkRegisteredUser($this->request->post)){
			...
			echo('$("'.$prefix_eval.' #subscribe_result").html("'.$this->language->get('subscribe').'");$("'.$prefix_eval.' #subscribe")[0].reset();');
		     ...
		     echo('$("'.$prefix_eval.' #subscribe_result").html("'.$this->language->get('subscribe').'");$("'.$prefix_eval.' #subscribe")[0].reset();');
				...
				$to1  = $this->request->post['subscribe_email'] ;
				$subject1 = $this->language->get('text_nlsubs_subj'); 
				$message1 = ' 
				<html> 
					<head> 
						<title>' . $this->language->get('text_nlsubs_title') . '</title> 
					</head> 
					<body> 
						<p>' . $this->language->get('text_nlsubs_text') . '</p> 
					</body> 
				</html>'; 
				$headers1  = "Content-type: text/html; charset=utf-8 \r\n"; 
				$headers1 .= "From: " . $this->language->get('text_nlsubs_from') . "\r\n"; 
				//$headers1 .= "Bcc: no-replay@site.ua\r\n"; 
				mail($to1, $subject1, $message1, $headers1); 
				
				$to2  = $this->language->get('text_nlsubs_mailto');
				$subject2 = $this->language->get('text_nlsubs_newsubs'); 
				$message2 = ' 
				<html> 
					<head> 
						<title>' . $this->language->get('text_nlsubs_titleadm') . '</title> 
					</head> 
					<body> 
						<p>' . $this->language->get('text_nlsubs_textadm') . '</p> 
					</body> 
				</html>'; 
				$message2 .= $this->request->post['subscribe_email']; 
				$headers2  = "Content-type: text/html; charset=utf-8 \r\n"; 
				$headers2 .= "From: " . strtoupper($_SERVER['HTTP_HOST']) . " Подписка <no-replay@" . $_SERVER['HTTP_HOST'] . ">\r\n"; 
				//$headers2 .= "Bcc: no-replay@site.ua\r\n"; 
