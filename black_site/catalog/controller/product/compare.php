...
/*		if (isset($this->request->get['clear'])) { // если кнопка очищаем нажата
		...
		$this->data['text_compare_title'] = $this->language->get('text_compare_title');
		$this->data['text_compare_addtocomp'] = $this->language->get('text_compare_addtocomp');
		$this->data['text_compare_clear'] = $this->language->get('text_compare_clear');
		$this->data['text_compare_zak'] = $this->language->get('text_compare_zak');
		$this->data['text_compare_xarakt'] = $this->language->get('text_compare_xarakt');
		$this->data['text_compare_absent'] = $this->language->get('text_compare_absent');
        ...
        $this->data['text_equal_diff'] = $this->language->get('text_equal_diff');
        $this->data['text_not_enough_products'] = $this->language->get('text_not_enough_products');
		...
		foreach ($this->session->data['compare'] as $key => $product_id) {
			...
			if ($product_info) {
                ...
                $product_category = $this->model_catalog_product->getCategoryByProduct($product_id);
                ...
                $category_array[] = array($product_category['category_id'] => $product_category['category_name']);
        ...
        foreach($category_array as $category_array1) {
            foreach ($category_array1 as $key => $category) {
                if(!in_array($category, $uniqueArray)) {
                    $uniqueArray[$key] = $category;
                }
            }
        }

        $this->data['categories'] = $uniqueArray;

        $this->data['categories_links'] = array();

        foreach ($uniqueArray as $key => $category) {
            $this->data['categories_links'][] = array(
                'cat_id'   => $key,
                'cat_name' => $category,
                'cat_href' => $this->url->link('product/category', 'path=' . $key)
            );
        }

        //print_r($this->data['categories_links']);
