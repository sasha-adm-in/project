...
class ControllerRecordBlog extends Controller
{
	public function index()
	{
        ...
        $this->data['informations'] = array();
			...
			$this->document->addLink($this->url->link('record/blog', 'blog_id=' . $this->request->get['blog_id']), 'canonical');
		...
		$this->data['text_blog_date'] = $this->language->get('text_blog_date');
		...
		if ($this->config->get('generallist') != '') {
			...
			$this->data['generallist'] = Array();
			$this->config->set('generallist', $this->data['generallist']);
		}
		...
		$sort      = 'p.sort_order';
		if (isset($this->data['generallist']['order']) && in_array($this->data['generallist']['order'], $sort_data)) {
			if ($this->data['generallist']['order'] == 'rating') {
				$sort = 'rating';
			} //$this->data['generallist']['order'] == 'rating'
			if ($this->data['generallist']['order'] == 'comments') {
				$sort = 'comments';
			} //$this->data['generallist']['order'] == 'comments'
			if ($this->data['generallist']['order'] == 'latest') {
				$sort = 'p.date_available';
			} //$this->data['generallist']['order'] == 'latest'
			if ($this->data['generallist']['order'] == 'sort') {
				$sort = 'p.sort_order';
			} //$this->data['generallist']['order'] == 'sort'
			if ($this->data['generallist']['order'] == 'popular') {
				$sort = 'p.viewed';
			} //$this->data['generallist']['order'] == 'popular'
		} //isset($this->data['generallist']['order']) && in_array($this->data['generallist']['order'], $sort_data)
		$order = 'DESC';
		if (isset($this->data['generallist']['order_ad'])) {
			if (strtoupper($this->data['generallist']['order_ad']) == 'ASC') {
				$order = 'ASC';
			} //strtoupper($this->data['generallist']['order_ad']) == 'ASC'
			if (strtoupper($this->data['generallist']['order']) == 'DESC') {
				$order = 'DESC';
			} //strtoupper($this->data['generallist']['order']) == 'DESC'
		} //isset($this->data['generallist']['order_ad'])
		...
		if ($blog_info) {
			...
			foreach ($results as $result) {
				if ($result['image']) {
                    ...
                    if ($dimensions['height'] == '')
                        $dimensions['height'] = 200;
                    $image                   = $this->model_tool_image->resize('no_image.jpg', $dimensions['width'], $dimensions['height']);
                    $this->data['image_dim'] = $dimensions;
				}
				...
				$this->data['records'][] = array(
					...
					'settings' => $this->data['generallist'],
			...
			$this->data['sorts'][] = array(
				...
				'value' => 'p.p.date_available-ASC',
				'href' => $this->url->link('record/blog', 'blog_id=' . $this->request->get['blog_id'] . '&sort=p.p.date_available&order=ASC' . $url)
			);
			$this->data['sorts'][] = array(
			...
			if (isset($this->request->get['rss'])) {
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/record/blogrss.tpl')) {
					$this->template = $this->config->get('config_template') . '/template/record/blogrss.tpl';
				} //file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/record/blogrss.tpl')
				else {
					if (file_exists(DIR_TEMPLATE . 'default/template/record/blogrss.tpl')) {
						$this->template = 'default/template/record/blogrss.tpl';
