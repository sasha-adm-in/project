...
MegaFilter.prototype = {
	...
	*/
		...
		self._box.find('.mfilter-category-tree').each(function(){
			...
			_this.find('.mfilter-to-parent a').unbind('click').bind('click', function(){
				...
				if( top_url != '' && parts.length <= top_path.length ) {
					window.location.href = self.createUrl(true, top_url);
				} else {
	...
	_initCountInfo: function() {
		...
		jQuery.ajax({
			'url'		: self._ajaxUrl( self.createUrl( false, self._options.ajaxInfoUrl, jQuery.extend( {}, self._params ), true ) ),
			'type'		: 'GET',
			'data'		: {
	...
	__initLoader: function() {
		this._jqLoader = jQuery('<span style="cursor: wait; z-index: 100; margin: 0; padding: 0; position: absolute; text-align: center; background-color: rgba(255,255,255,0.7);"></span>')
			...
			.html( '<img src="catalog/view/theme/res_final/stylesheet/mf/images/ajax-loader.gif" alt="" />' )
			.hide();
	},
	...
	_render: function( html, json, history ) {
		...
		if( history !== true ) {
			self._lastUrl = self.createUrl(false);
			self._urlToFilters = null;
	...
	ajax: function() {
		...
		var url		= [ self._options.routeProduct, self._options.routeHome, self._options.routeInformation ].indexOf( self._options.route ) > -1 ? self.createUrl( false, self._options.ajaxResultsUrl ) : self.createUrl(false),
	 ...
	 */
	createUrl: function( reload, url, attribs, force ) {
		var self	= this,
			params	= self.paramsToUrl( url, attribs ),
			filters	= self.filtersToUrl(),
			urlSep	= self._urlSep;
		if (reload) { 
			var rr = filters.match(/manufacturers\[\d{1,}\]/gi);
			if (rr) {
				filters = filters.replace(rr, '');	
				if (filters.charAt(0) === ',') {
	...
	reload: function() {
		...
		window.location.href = self.createUrl(true);
	}
};
var MegaFilterLang = {};
