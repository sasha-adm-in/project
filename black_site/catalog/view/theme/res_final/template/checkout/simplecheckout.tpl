...
<div id="content-wrapper">
    <div class="section cart_content">
...
<?php } ?>
    ...
    <!-- simplecheckout form -->
        <?php 
            $replace = array(
                '{three_column}'  => '<div id="pay_method">',
                '{/three_column}' => '</div>',
                '{left_column}'   => '<div class="one-half"><div class="contact_data">',
                '{/left_column}'  => '</div></div>',
                '{right_column}'  => '<div class="one-half last">',
                '{/right_column}' => '</div>',
                ...
                '{shipping}'      => $has_shipping ? '<div class="simplecheckout-block'.($simple_shipping_methods_hide ? ' simplecheckout-skip' : '').'" id="simplecheckout_shipping"'.($simple_shipping_methods_hide ? ' style="display:none;"' : '').'>' . $simplecheckout_shipping . '</div></div>' : '',
                '{payment}'       => '<div class="payment_method"><div class="simplecheckout-block'.($simple_payment_methods_hide ? ' simplecheckout-skip' : '').'" id="simplecheckout_payment"'.($simple_payment_methods_hide ? ' style="display:none;"' : '').'>' . $simplecheckout_payment . '</div>',
            ...
            if ($payment_form) {
                $replace['{payment_form}'] = '<div class="simplecheckout-block" id="simplecheckout_payment_form" style="display:none"> ';
                $replace['{payment_form}'] .= '<div class="simplecheckout-block-heading">' . $text_payment_form_title . '</div>';
                $replace['{payment_form}'] .= '<div class="simplecheckout-block-content" id="payout">' . $payment_form . '</div>';
    ...
    <!--<div class="simplecheckout-button-block buttons" id="buttons" <?php if ($block_order) { ?>style="display:none;"<?php } ?>>
        <div class="simplecheckout-button-right">
            <?php if ($simple_common_view_agreement_checkbox) { ?><label><input type="checkbox" id="agree" name="agree" value="1" <?php if ($agree == 1) { ?>checked="checked"<?php } ?> /><?php echo $text_agree; ?></label>&nbsp;<?php } ?><a class="button btn" style="display:none;" onclick="simplecheckout_submit();" id="simplecheckout_button_confirm"><span><?php echo $button_order; ?></span></a>
        </div>
...
<script>
//$('<div class="payment_method">').insertBefore('#simplecheckout_payment');
if (document.getElementById('without')) {
    $.cookie('type', 'customer1');
	$('#no_reg').trigger('click');
	$('#custmomer1').trigger('click');
    
    document.getElementById('without').checked = true;
} else {
    document.getElementById('with_fiz').checked = true;
}

$(document).on('change', 'input:radio[name="customer_group_id2"]', function() {
	if($(this).val() == '1'){
		$.cookie('type', 'customer1');
		$('#no_reg').trigger('click');
		$('#custmomer1').trigger('click');
	...
	if($(this).val() == '2'){
		$.cookie('type', 'customer2');
		$('#yes_reg').trigger('click');
		$('#custmomer1').trigger('click');
	}
	if($(this).val() == '3'){
		$.cookie('type', 'customer3');
		$('#yes_reg').trigger('click');
		$('#custmomer2').trigger('click');
	}
