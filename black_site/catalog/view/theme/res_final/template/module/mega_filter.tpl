<?php print_r($filters[0]['options']); if( ! empty( $settings['javascript'] ) ) { ?>
...
<?php
	...
	$button_reset_temp = "<a onclick=\"window.location.href='" . $current_category . "';return false;\" href=\"#\" class=\"%s\">%s</a>";
	$buttons = array( 'top' => array(), 'bottom' => array() );
	
	if( ! empty( $settings['show_reset_button'] ) ) {
		$buttons['bottom'][] = sprintf( $button_reset_temp, 'mfilter-button-reset-custom', '<i class="mfilter-reset-icon"></i>' . $text_reset_all );
	}
	
	if( ! empty( $settings['show_top_reset_button'] ) ) {
		$buttons['top'][] = sprintf( $button_reset_temp, 'mfilter-button-reset-custom', '<i class="mfilter-reset-icon"></i>' . $text_reset_all );
	}
											...
											<?php for( $i = 5; $i >= 1; $i-- ) { ?>
												<div class="mfilter-option mfilter-tb-as-tr">
													...
													<label class="mfilter-tb-as-td" for="mfilter-opts-rating-<?php echo (int) $_idx; ?>-<?php echo $i; ?>">
														<?php echo $i; ?> <img src="catalog/view/theme/res_final/image/stars-<?php echo $i; ?>.png" alt="" />
													</label>
									...
									<?php } else if( in_array( $filter['type'], array( 'stock_status', 'manufacturers', 'checkbox', 'radio', 'image_list_radio', 'image_list_checkbox' ) ) ) { ?>
										...
										<div class="mfilter-options-container">
											...
											<?php foreach( $filter['options'] as $option_id => $option ) { if( $option['name'] === '' || isset( $options_tmp[$option['key']] ) ) continue; $options_tmp[$option['key']] = true; ?>
												...
												<div class="mfilter-option mfilter-tb-as-tr">
													<div <?php if (isset($option['href'])) { ?>style="display: none;"<?php } ?> class="mfilter-tb-as-td mfilter-col-input">
														<input
													...
													<label class="mfilter-tb-as-td" for="mfilter-opts-attribs-<?php echo (int) $_idx; ?>-<?php echo $base_id; ?>-<?php echo $option['key']; ?>">
														...
														<?php } else { ?>
															<?php if (isset($option['href'])) { ?>
																<span><?php echo $option['name']; ?></span>
															    <a href="<?php echo $option['href']; ?>"><?php echo $option['name']; ?></a>
															<?php } else { ?>
																<?php echo $option['name']; ?>
															<?php } ?>
														<?php } ?>
													</label>
													<div <?php if (isset($option['href'])) { ?>style="display: none;"<?php } ?> class="mfilter-tb-as-td mfilter-col-count"><span class="mfilter-counter">0</span></div>
												</div>
