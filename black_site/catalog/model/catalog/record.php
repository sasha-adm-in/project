...
class ModelCatalogRecord extends Model
{
	...
	public function getRelatedRecords($product_id, $data)
	{
	 if ($product_id) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		...
		$record_data = array();
		$sql         = "SELECT * FROM " . DB_PREFIX . "record_product_related pr
...
//			}
	...
	{
		$product_data = array();
		$query        = $this->db->query("SELECT * FROM " . DB_PREFIX . "record_product_related pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.record_id = '" . (int) $record_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int) $this->config->get('config_store_id') . "'");
		foreach ($query->rows as $result) {
	...
	public function getBlogsRecords($data = array())
	{
		...
		if (!$record_data) {
			...
			WHERE r1.record_id = p.record_id AND r1.status = '1' GROUP BY r1.record_id) AS rating
			FROM " . DB_PREFIX . "record p
			LEFT JOIN " . DB_PREFIX . "record_to_blog r2b ON (p.record_id = r2b.record_id)
			LEFT JOIN " . DB_PREFIX . "record_description pd ON (p.record_id = pd.record_id)
			LEFT JOIN " . DB_PREFIX . "record_to_store p2s ON (p.record_id = p2s.record_id)";
			$sql .= " WHERE
